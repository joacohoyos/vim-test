...
function! test#javascript#reactscripts#executable() abort
  if filereadable('node_modules/.bin/react-app-rewired')
    return 'node_modules/.bin/react-app-rewired test --env=jsdom --coverage --watchAll=false -u'
  elseif filereadable('node_modules/.bin/react-scripts')
    return 'node_modules/.bin/react-scripts test --env=jsdom --coverage --watchAll=false -u'
  else
    return 'react-scripts test --env=jsdom --coverage --watchAll=false -u'
  endif
endfunction
